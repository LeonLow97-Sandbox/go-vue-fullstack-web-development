...
const dbTimeout = time.Second * 3 // for context timeout
...
// Update updates 1 user in the db, using the information stored in the receiver u
...
func (t *Token) GenerateToken(userID int, ttl time.Duration) (*Token, error) {
	token := &Token{
		UserID: userID,
		Expiry: time.Now().Add(ttl), // ttl - time to life
	}
	...
	// generates the actual token itself (will be exactly 26 characters long)
...
func (t *Token) AuthenticateToken(r *http.Request) (*User, error) {
	// Check for Authorization header from frontend
...
func (t *Token) Insert(token Token, u User) error {
	...
	// delete any existing tokens (avoid having multiple tokens for a specific user)
